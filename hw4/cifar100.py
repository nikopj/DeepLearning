...
def conv_layer(input, filters, kernel_size, strides=2, is_training=True):
    ...
    x = tf.layers.batch_normalization(x, training=phase, renorm=False)
    x = tf.nn.relu6(x)
    return x
...
def f(x):
    x = conv_layer(x, 32, 3, strides=2, is_training=phase)
    print(x.get_shape())
    x = conv_layer(x, 32, 3, strides=2, is_training=phase)
    print(x.get_shape())
    ...
    x = fc_layer(x, 64, is_training=phase)
...
# FROM KAROL
trash, guess_5 = tf.nn.top_k(prediction, k=5)
actual = tf.argmax(y, axis=1)
actual = tf.transpose(tf.stack([actual,actual,actual,actual,actual]))
correct_pred_5 = tf.equal(guess_5, tf.cast(actual, tf.int32))
accuracy_5 = 5.0*tf.reduce_mean(tf.cast(correct_pred_5, tf.float32))
...
with tf.Session() as sess:
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        print('TOP-1 Validation Set Accuracy:',
            accuracy.eval({x: data.x_val, y: data.y_val, phase: False}))

        print('TOP-5 Validation Set Accuracy:',
            accuracy_5.eval({x: data.x_val, y: data.y_val, phase: False}))

    # Test the model on separate data
    print('TOP-1 Test Set Accuracy:',
        accuracy.eval({x: data.x_test, y: data.y_test, phase: False}))

    print('TOP-5 Test Set Accuracy:',
        accuracy_5.eval({x: data.x_test, y: data.y_test, phase: False}))
